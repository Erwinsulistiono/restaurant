...
class Inventory extends MY_Controller
{
  ...
  {
    ($dataBase) ? $db = $dataBase : $db = $this->input->post('selectDb');
    $data = [
      'dataBase' => $db,
      'outlet' => $this->M_crud->read('tbl_outlet'),
      'kategori' => $this->M_crud->read('tbl_kategori'),
      'data' => $this->M_crud->read('tbl_stock_' . $db . '  ' . ' ORDER BY ' . '\'stock_kat\''),
    ];
  ...
  public function simpan_inventory($dataBase)
  {
    ...
    if (!$stock_id) {
      $this->M_crud->insert('tbl_stock_' . $dataBase, $data);
      ...
      redirect('admin/inventory/outlet/' . $dataBase);
    }
    $data_old = $this->M_crud->select('tbl_stock_' . $dataBase, 'stock_id', $stock_id);
    $log_oldval = strtr(json_encode($data_old), array(',' => ' | ', '{' => '', '}' => '', '"' => ''));

    $this->M_crud->update('tbl_stock_' . $dataBase, $data, 'stock_id', $stock_id);
    $this->M_log->simpan_log($stock_id, 'INVENTORY', $log_oldval, $log_newval);
    $this->session->set_flashdata('msg', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><b>' . $data['nm_brg'] . '</b> Berhasil ditambah/diupdate.</div>');
    redirect('admin/inventory/outlet/' . $dataBase);
  }

  public function hapus_inventory($dataBase, $stock_id)
  {
    $data_old = $this->M_crud->select('tbl_stock_' . $dataBase, 'stock_id', $stock_id);
    $log_oldval = strtr(json_encode($data_old), array(',' => ' | ', '{' => '', '}' => '', '"' => ''));

    $this->M_log->simpan_menu($stock_id, 'INVENTORY', $log_oldval);
    $this->M_crud->delete('tbl_stock_' . $dataBase, 'stock_id', $stock_id);
  ...
  public function tambah_inventory($dataBase)
  {
    $stock_id = $this->input->post('stock_id');
    ...
    $this->M_crud->update('tbl_stock_' . $dataBase, $data, 'stock_id', $stock_id);
    $data_new = $this->M_crud->select('tbl_stock_' . $dataBase, 'stock_id', $stock_id);
  ...
  public function transfer_inventory($dataBase)
  {
    $stock_id = $this->input->post('stock_id');
    $db_target = $this->input->post('db_target');
    $add_stock = $this->input->post('stock_qty');
    $source_data = $this->M_crud->select('tbl_stock_' . $dataBase, 'stock_id', $stock_id);
    $target_data = $this->M_crud->select('tbl_stock_' . $db_target, 'stock_reffid', $stock_id);
    $qty_old['stock_qty'] = $source_data['stock_qty'] - $add_stock;
    $this->M_crud->update('tbl_stock_' . $dataBase, $qty_old, 'stock_id', $stock_id);
    ...
    if (!$target_data) {
      ...
      $this->M_crud->insert('tbl_stock_' . $db_target, $source_data);
      $reff_id = $this->db->insert_id();
    } else {
      ...
      $this->M_crud->update('tbl_stock_' . $db_target, $qty_new, 'stock_reffid', $stock_id);
    }
    $log_newval = strtr(json_encode($this->M_crud->select('tbl_stock_' . $db_target, 'stock_reffid', $stock_id)), array(',' => ' | ', '{' => '', '}' => '', '"' => ''));
