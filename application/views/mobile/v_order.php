  ...
  <script>
    ...
    simpanPembayaran.forEach(bayar => {
      bayar.addEventListener('click', (e) => {
        ...
        const customerId = ((sessionStorage.getItem('dataPelanggan')) ? JSON.parse(sessionStorage.getItem('dataPelanggan')).plg_id : 0);
        const db = JSON.parse(sessionStorage.getItem('db'))
        let payment_id = (event.currentTarget.dataset.paymentid) ? event.currentTarget.dataset.paymentid : 'Open Table';
        let payment_nama = (event.currentTarget.dataset.paymentnama) ? event.currentTarget.dataset.paymentnama : 'Open Table';
