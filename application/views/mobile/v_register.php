            ...
            <div class="card-body" style="padding-top:20%">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <form class='form floating-label' action='<?= base_url('mobile/order/auth/') . $outlet ?>' accept-charset='utf-8' method='post'>
...
<script type="text/javascript">
    $('#selectMethodOfTable').change(function() {
        ...
        let divElement = '';
        let meja = JSON.parse('<?php echo json_encode($data); ?>');

        if (methodType == 1) {
            divElement +=
                `<select name="meja_pelanggan" class="form-control dirty">
                    <option value="">&nbsp;</option>`;
            Object.values(meja).forEach(m => {
                divElement += (m.meja_pelanggan == 0) ? `<option value="${m.meja_id}">${m.meja_nama}</option>` : '';
            })
